...
public class Diff {
        ...
        if (ra.getClass() != rb.getClass()) {
          ...
          if (path.endsWith(".jar")) {
            ...
            try {
              ...
              if (!result.isEmpty()) different.put(path, result);
            }
            finally {
              aa.close();
              bb.close();
            ...
            String cmp = diff(ra.openInputStream(), rb.openInputStream());
            if (cmp != null) different.put(path, cmp);
          }
        }
      }
    }
  ...
  public void compareManifest(Map<String, Object> different, Manifest a, Manifest b, boolean strict) {
    ...
    for (Object element : attrs.keySet()) {
      ...
      if (bv != null) {
        if (!av.equals(bv)) different.put(name.toString(), "M:" + name + ":" + av + "!=" + bv);
      }
    }
  }
