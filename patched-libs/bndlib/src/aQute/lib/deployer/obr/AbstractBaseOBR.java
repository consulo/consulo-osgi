...
public abstract class AbstractBaseOBR implements RegistryPlugin, Plugin, RemoteRepositoryPlugin, OBRIndexProvider {
  ...
  protected final synchronized void init() throws Exception {
    if (!initialised) {
      ...
      IRepositoryListener listener = new IRepositoryListener() {
        ...
        public boolean processReferral(String fromUrl, Referral referral, int maxDepth, int currentDepth) {
          try {
            ...
            try {
              CachingURLResourceHandle indexHandle =
                new CachingURLResourceHandle(indexLocation.toExternalForm(), null, getCacheDirectory(), connector,
      ...
      for (URL indexLocation : indexes) {
        try {
          CachingURLResourceHandle indexHandle =
            new CachingURLResourceHandle(indexLocation.toExternalForm(), null, getCacheDirectory(), connector, CachingMode.PreferRemote);
          indexHandle.setReporter(reporter);
  ...
  public synchronized void setProperties(Map<String, String> map) {
    if (map.containsKey(PROP_NAME)) name = map.get(PROP_NAME);
  ...
  protected ResourceHandle[] getHandles(String bsn, String rangeStr) throws Exception {
    ...
    if ("project".equals(rangeStr)) return null;


    SortedMap<Version, Resource> versionMap = bsnMap.get(bsn);
    if (versionMap == null || versionMap.isEmpty()) return null;
  ...
  public List<String> list(String regex) throws Exception {
    ...
    for (String bsn : bsnMap.keySet()) {
      if (pattern == null || pattern.matcher(bsn).matches()) result.add(bsn);
    }
    ...
    for (Capability capability : resource.getCapabilities()) {
      if (CapabilityType.PACKAGE.getTypeName().equals(capability.getName())) {
        ...
        for (Property prop : capability.getProperties()) {
          ...
          else if (Property.VERSION.equals(prop.getName())) versionStr = prop.getValue();
        }
   ...
   * @throws IOException
   */
  boolean readIndex(String baseUrl, InputStream stream, IRepositoryListener listener)
  ...
  List<Resource> narrowVersionsByFilter(String pkgName, SortedMap<Version, Resource> versionMap, Filter filter) {
    ...
    for (Version version : versionMap.keySet()) {
      ...
      if (filter.match(dict)) result.add(versionMap.get(version));
    }
  ...
  List<Resource> narrowVersionsByVersionRange(SortedMap<Version, Resource> versionMap, String rangeStr) {
    ...
    if ("latest".equals(rangeStr)) {
      ...
      if (range != null && range.getLow() != null) versionMap = versionMap.tailMap(range.getLow());

      result = new ArrayList<Resource>(versionMap.size());
      for (Version version : versionMap.keySet()) {
        if (range == null || range.includes(version)) result.add(versionMap.get(version));
    ...
    for (Iterator<Resource> iter = resources.iterator(); iter.hasNext(); ) {
      ...
      if (modeRequire == null) {
        ...
        try {
          ...
          if (!filter.match(modeCapability)) iter.remove();
        }
  ...
  ResourceHandle mapResourceToHandle(Resource resource) throws Exception {
    ...
    try {
      handle = new CachingURLResourceHandle(resource.getUrl(), resource.getBaseUrl(), getCacheDirectory(), getConnector(),
                                            CachingMode.PreferCache);
    }
    catch (FileNotFoundException e) {
      throw new FileNotFoundException("Broken link in repository index: " + e.getMessage());
    }
    if (handle.getLocation() == Location.local || getCacheDirectory() != null) result = handle;
  ...
  ResourceHandle resolvePackage(String pkgName, String rangeStr, Strategy strategy, ResolverMode mode, Map<String, String> props)
    throws Exception {
    init();
    if (rangeStr == null) rangeStr = "0.0.0";
    ...
    if (mode != null) filterResourcesByResolverMode(resources, mode);
    ...
    if (usesProp != null) {
      ...
      while (tokenizer.hasMoreTokens()) {
        ...
        if (usesPkgCap != null) {
          ...
          if (require != null) externalUses.put(usesPkgName, require);
        }
      }
    }
