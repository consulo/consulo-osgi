...
public class Clazz {
  ...
  public static enum JAVA {
    UNKNOWN(Integer.MAX_VALUE), OpenJDK7(51), J2S6(50), J2SE5(49), JDK1_4(48), JDK1_3(47), JDK1_2(46), JDK1_1(45);
  ...
  public static EnumSet<QUERY> HAS_ARGUMENT =
    EnumSet.of(QUERY.IMPLEMENTS, QUERY.EXTENDS, QUERY.IMPORTS, QUERY.NAMED, QUERY.VERSION, QUERY.ANNOTATION);
  ...
  static public class FieldDef implements Comparable<FieldDef> {
    ...
    public boolean equals(Object other) {
      if (!(other instanceof MethodDef)) return false;
  ...
  static public class MethodDef extends FieldDef {
    ...
    public String getReturnType() {
      ...
      if (signature != null) use = signature;

      Matcher m = METHOD_DESCRIPTOR.matcher(use);
      if (!m.matches()) throw new IllegalArgumentException("Not a valid method descriptor: " + descriptor);
  ...
  Set<String> parseClassFile(DataInputStream in) throws IOException {
    ...
    try {
      ...
      if (cd != null) {
        if (!cd.classStart(access_flags, className)) return null;
      }
      ...
      if (zuper != null) {
        ...
        if (cd != null) cd.extendsClass(zuper);
      }
      ...
      if (interfacesCount > 0) {
        ...
        if (cd != null) cd.implementsInterfaces(interfaces);
      }
      ...
      for (int i = 0; i < fieldsCount; i++) {
        ...
        if (cd != null) cd.field(last = new FieldDef(access_flags, className, name, pool[descriptor_index].toString()));
      ...
      if (crawl) {
        ...
        class$ = findMethodReference(className, "class$", "(Ljava/lang/String;)Ljava/lang/Class;");
      }
      else if (major == 48) {
        forName = findMethodReference("java/lang/Class", "forName", "(Ljava/lang/String;)Ljava/lang/Class;");
        if (forName > 0) {
  ...
  private void constantInteger(DataInputStream in, int poolIndex) throws IOException {
    ...
    if (cd != null) pool[poolIndex] = intPool[poolIndex];
  }
  ...
  private void doInnerClasses(DataInputStream in) throws IOException {
    ...
    for (int i = 0; i < number_of_classes; i++) {
      ...
      if (cd != null) {
        ...
        if (inner_name_index != 0) innerName = (String)pool[inner_name_index];
  ...
  void doSignature(DataInputStream in, ElementType member) throws IOException {
    ...
    if (member != ElementType.TYPE) parseDescriptor(signature);

    if (last != null) last.signature = signature;

    if (cd != null) cd.signature(signature);
  }
    ...
    while (bb.remaining() > 0) {
      ...
      switch (instruction) {
        ...
        case OpCodes.invokespecial: {
          ...
          if (cd != null) cd.reference(getMethodDef(0, mref));
          break;
        }
        ...
        case OpCodes.invokestatic: {
          ...
          if (cd != null) cd.reference(getMethodDef(0, methodref));
          ...
          if ((methodref == forName || methodref == class$) && lastReference != -1 && pool[intPool[lastReference]] instanceof String) {
            ...
            if (clazz.startsWith("[") || clazz.endsWith(";")) {
              ...
              if (n > 0) packageReference(clazz.substring(0, n));
            }
          }
          break;
        }
  ...
  private Annotation doAnnotation(DataInputStream in, ElementType member, RetentionPolicy policy, boolean collect) throws IOException {
    ...
    if (annotations == null) annotations = new HashSet<String>();
    ...
    for (int v = 0; v < num_element_value_pairs; v++) {
      ...
      if (collect) {
        if (elements == null) elements = new LinkedHashMap<String, Object>();
  ...
  private Object doElementValue(DataInputStream in, ElementType member, RetentionPolicy policy, boolean collect) throws IOException {
    ...
    switch (tag) {
      ...
      case 'e': // enum constant
        ...
        if (policy == RetentionPolicy.RUNTIME) descriptors.add(new Integer(type_name_index));
        int const_name_index = in.readUnsignedShort();
        return pool[const_name_index];

      case 'c': // Class
        int class_info_index = in.readUnsignedShort();
        if (policy == RetentionPolicy.RUNTIME) descriptors.add(new Integer(class_info_index));
   ...
   *
   * @param pack A '.' delimited package name
   */
  ...
  int parseReference(String descriptor, int rover) {
    ...
    while (c == '[') c = descriptor.charAt(++rover);
    ...
    if (c == '<') {
      ...
      // Type variable name
      rover++;
      while (descriptor.charAt(rover) != ';') rover++;
    }
    else if (c == 'L') {
      StringBuilder sb = new StringBuilder();
      rover++;
      ...
      if (cd != null) cd.addReference(sb.toString());

      if (lastSlash > 0) packageReference(sb.substring(0, lastSlash));
    }
    else {
      if ("+-*BCDFIJSZV".indexOf(c) < 0) ;// System.out.println("Should not skip: " + c);
    }
  ...
  String getClassName() {
    if (className == null) return "NOCLASSNAME";
    return className;
  }
  ...
  public boolean is(QUERY query, Instruction instr, Analyzer analyzer) throws Exception {
    switch (query) {
      ...
      case ANNOTATION:
        ...
        if (annotations.contains(instr.getPattern())) return true;

        for (String annotation : annotations) {
          if (instr.matches(annotation)) return !instr.isNegated();
        }
  ...
  public static void getImplementedPackages(Set<String> implemented, Analyzer analyzer, Clazz clazz) throws Exception {
    if (clazz.interfaces != null) {
      for (String interf : clazz.interfaces) {
        ...
        if (c == null) c = analyzer.findClass(interf);
        ...
        if (c != null) {
          ...
          if (!consumer) implemented.add(getPackage(interf));
  ...
  public static String objectDescriptorToFQN(String string) {
    if (string.startsWith("L") && string.endsWith(";")) return string.substring(1, string.length() - 1).replace('/', '.');
  ...
  public static String unCamel(String id) {
    ...
    for (int i = 0; i < id.length(); i++) {
      ...
      if (n == i) {
        ...
        if (i > 0 && !Character.isWhitespace(out.charAt(out.length() - 1))) out.append(' ');
