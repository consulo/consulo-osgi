...
public class ComponentDef {
  ...
  void prepare(Analyzer analyzer) {
    ...
    for (ReferenceDef ref : references.values()) {
      ...
      if (ref.version.compareTo(version) > 0) version = ref.version;
    }
  ...
  Tag getTag() {
    ...
    if (servicefactory != null) component.addAttribute("servicefactory", servicefactory);

    if (configurationPolicy != null) component.addAttribute("configuration-policy", configurationPolicy.toString().toLowerCase());
    ...
    if (activate != null) component.addAttribute("activate", activate);

    if (deactivate != null) component.addAttribute("deactivate", deactivate);
    ...
    if (service != null && service.length != 0) {
      ...
      if (servicefactory != null && servicefactory) s.addAttribute("servicefactory", true);
