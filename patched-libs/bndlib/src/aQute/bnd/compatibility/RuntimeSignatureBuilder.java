...
public class RuntimeSignatureBuilder {
  ...
  public Scope add(Class<?> c) {
    Scope local =
      add(root, getEnclosingScope(c), c.getModifiers(), c.getTypeParameters(), Kind.CLASS, identity(c), c.getGenericSuperclass(),
          c.getGenericInterfaces(), null);
...
//		}
  ...
  private Scope add(Scope declaring,
                    Scope enclosing,
                    int modifiers,
  ...
  private GenericType convert(Scope source, Type t) {
    if (t instanceof ParameterizedType) {
      ...
      ParameterizedType pt = (ParameterizedType)t;
                        /*Scope reference =*/
      root.getScope(identity((Class<?>)pt.getRawType()));
...
//			raw = ((Class<?>) t).getName() + ";";
  ...
  private GenericType[] convert(Type[] parameterTypes) {
    if (parameterTypes == null || parameterTypes.length == 0) return GenericType.EMPTY;
