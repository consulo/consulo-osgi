...
public class MavenDeployCmd extends Processor {
  ...
  public void setProperties(Map<String, String> map) {
    ...
    if (url == null) throw new IllegalArgumentException("MavenDeploy plugin must get a repository URL");
    if (repository == null) throw new IllegalArgumentException("MavenDeploy plugin must get a repository name");
  }
  ...
  public boolean deploy(Project project, Jar original) throws Exception {
    Map<String, Map<String, String>> deploy = project.parseHeader(project.getProperty(Constants.DEPLOY));

    Map<String, String> maven = deploy.get(repository);
    if (maven == null) return false; // we're not playing for this bundle
    ...
    if (manifest == null) {
      ...
      try {
        ...
        Map<String, Map<String, String>> exports = project.parseHeader(manifest.getMainAttributes().getValue(Constants.EXPORT_PACKAGE));
