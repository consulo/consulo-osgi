...
public class ComponentAnnotationReader extends ClassDataCollector {
  ...
  public void annotation(Annotation annotation) {
    ...
    if (annotation.getName().equals(Component.RNAME)) {
      ...
      if (configurationPolicy != null) set(COMPONENT_CONFIGURATION_POLICY, configurationPolicy.toLowerCase(), "");
      ...
      if(map == null) return;
      if (!checkMethod()) setVersion(V1_1);
      ...
      if (method.equals("activate") && OLDACTIVATEDESCRIPTOR.matcher(methodDescriptor).matches()) {
        // this is the default!
      }
      else {
        setVersion(V1_1);
      ...
      if (method.equals("deactivate") && OLDACTIVATEDESCRIPTOR.matcher(methodDescriptor).matches()) {
        // This is the default!
      }
      else {
        setVersion(V1_1);
      ...
      if (bind != null) {
        name = name + "/" + bind;
        if (unbind != null) name = name + "/" + unbind;
      }
      ...
      if (map.containsKey(name)) {
        reporter.error("In component %s, Multiple references with the same name: %s. Previous def: %s, this def: %s", name, map.get(name),
                       service, "");
      }
      map.put(name, service);

      if (isTrue(annotation.get(Reference.MULTIPLE))) multiple.add(simpleName);
      if (isTrue(annotation.get(Reference.OPTIONAL))) optional.add(simpleName);
      if (isTrue(annotation.get(Reference.DYNAMIC))) dynamic.add(simpleName);

      if (!checkMethod()) {
        setVersion(V1_1);
      }
      else if (REFERENCEBINDDESCRIPTOR.matcher(methodDescriptor).matches() || !OLDBINDDESCRIPTOR.matcher(methodDescriptor).matches()) {
        setVersion(V1_1);
      }
    }
  }
    ...
    if (properties != null) {
      for (Object o : properties) {
        ...
        if (PROPERTY_PATTERN.matcher(p).matches()) {
          ...
          throw new IllegalArgumentException("Malformed property '" + p + "' on: " + annotation.get(Component.NAME));
        }
      }
    }
  }
  ...
  private void set(String string, Object object, Object deflt) {
    if (object == null || object.equals(deflt)) return;
