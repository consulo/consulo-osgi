...
public class Coverage {
   ...
   * @throws IOException
   */
  public static Map<MethodDef, List<MethodDef>> getCrossRef(Collection<Clazz> source, Collection<Clazz> dest) throws Exception {
    final Map<MethodDef, List<MethodDef>> catalog = buildCatalog(dest);
    crossRef(source, catalog);
    return catalog;
  }

  private static void crossRef(Collection<Clazz> source, final Map<MethodDef, List<MethodDef>> catalog) throws Exception {
    for (final Clazz clazz : source) {
      clazz.parseClassFileWithCollector(new ClassDataCollector() {
        MethodDef source;

        public void implementsInterfaces(String names[]) {
          MethodDef def = new MethodDef(0, clazz.getFQN(), "<implements>", "()V");
  ...
  private static Map<MethodDef, List<MethodDef>> buildCatalog(Collection<Clazz> sources) throws Exception {
    ...
    for (final Clazz clazz : sources) {
      clazz.parseClassFileWithCollector(new ClassDataCollector() {
        ...
        public void method(MethodDef source) {
          if (java.lang.reflect.Modifier.isPublic(source.access) || Modifier.isProtected(source.access)) {
