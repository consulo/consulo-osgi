...
public abstract class ProjectLauncher {
  ...
  private int getRunframework(String property) {
    ...
    else if (Constants.RUNFRAMEWORK_SERVICES.equalsIgnoreCase(property)) return SERVICES;
        ...
        if (!classpath.contains(path)) {
          ...
          if (manifest != null) {
            ...
            for (Map.Entry<String, Map<String, String>> e : exports.entrySet()) {
              if (!runsystempackages.containsKey(e.getKey())) runsystempackages.put(e.getKey(), e.getValue());
            }
            ...
            if (activator != null) activators.add(activator);
          }
        }
      }
    }
  ...
  public int launch() throws Exception {
    ...
    if (timeout != 0) java.setTimeout(timeout + 1000, TimeUnit.MILLISECONDS);

    int result = java.execute(System.in, System.out, System.err);
    if (result == Integer.MIN_VALUE) return TIMEDOUT;
