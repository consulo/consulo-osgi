...
public class Project extends Processor {
  ...
  final static String DEFAULT_ACTIONS =
    "build; label='Build', test; label='Test', run; label='Run', clean; label='Clean', release; label='Release', refreshAll; label=Refresh, deploy;label=Deploy";
   ...
   *
   * @param parent The project builder to use as parent, use this project if null
   * @return
   * @throws Exception
   */
  ...
  public synchronized void prepare() throws Exception {
    ...
    if (inPrepare) throw new CircularDependencyException(trail.toString() + "," + this);
    ...
    try {
      if (!preparedPaths) {
        ...
        try {
          ...
          else if (!buildpath.contains(output)) buildpath.add(new Container(this, output));
  ...
  public List<Container> getBundles(Strategy strategyx, String spec, String source) throws Exception {
    ...
    try {
      for (Iterator<Map.Entry<String, Map<String, String>>> i = bundles.entrySet().iterator(); i.hasNext(); ) {
        ...
        if (found == null) {
          if (versionRange != null && (versionRange.equals("project") || versionRange.equals("latest"))) {
            ...
            File f = getFile(bsn);
            String error = null;
            if (!f.exists()) error = "File does not exist: " + f.getAbsolutePath();
        ...
        if (found != null) {
          ...
          for (Container cc : libs) {
            if (result.contains(cc)) warning("Multiple bundles with the same final URL: " + cc);
          ...
          Container x =
            new Container(this, bsn, versionRange, Container.TYPE.ERROR, null, bsn + ";version=" + versionRange + " not found", attrs);
          result.add(x);
      ...
      if (source != null) message = String.format("%s (from property: %s)", message, source);
   ...
   *
   * @param strategyx The package-version disambiguation strategy.
   * @param spec      The value of the @{code -buildpackages} instruction.
   * @throws Exception
   */
  public void appendPackages(Strategy strategyx, String spec, List<Container> resolvedBundles, ResolverMode mode) throws Exception {
    ...
    while (!queue.isEmpty()) {
      ...
      if (found == null) found = getPackage(pkgName, versionRange, strategyx, attrs, mode);
      ...
      if (found != null) {
        if (resolvedBundles.contains(found)) {
          ...
          for (Container cc : libs) {
            ...
            if (importUses != null) queue.addAll(0, parseHeader(importUses).entrySet());
          }
        }
      }
      else {
  ...
  static void mergeNames(String names, Set<String> set) {
    ...
    while (tokenizer.hasMoreTokens()) set.add(tokenizer.nextToken().trim());
  }
  ...
  static void addToPackageList(Container container, String newPackageNames) {
    ...
    if (packageListStr != null) mergeNames(packageListStr, merged);
    if (newPackageNames != null) mergeNames(newPackageNames, merged);
   ...
   *
   * @param packageName The package required
   * @param range       The package version range required
   * @param strategyx   The package-version disambiguation strategy
   * @param attrs       Other attributes specified by the search.
   * @return
   * @throws Exception
   */
  public Container getPackage(String packageName, String range, Strategy strategyx, Map<String, String> attrs, ResolverMode mode)
    throws Exception {
    ...
    if (attrs == null) attrs = new HashMap<String, String>(2);
    ...
    return new Container(this, "X", range, Container.TYPE.ERROR, null,
                         "package " + packageName + ";version=" + range + " Not found in " + plugins, null);
  }
  ...
  private Strategy findStrategy(Map<String, String> attrs, Strategy defaultStrategy, String versionRange) {
    ...
    if (overrideStrategy != null) {
      ...
      else if ("exact".equalsIgnoreCase(overrideStrategy)) useStrategy = Strategy.EXACT;
    }
    if ("latest".equals(versionRange)) useStrategy = Strategy.HIGHEST;
    return useStrategy;
  }
  ...
  private void justInTime(Collection<Container> path, List<Container> entries) {
    if (delayRunDependencies && path.isEmpty()) doPath(path, dependson, entries, null);
  }
  ...
  public String _p_output(String args[]) throws Exception {
    if (args.length != 1) throw new IllegalArgumentException("${output} should not have arguments");
  ...
  public Container getBundle(String bsn, String range, Strategy strategy, Map<String, String> attrs) throws Exception {
    ...
    if (useStrategy == Strategy.EXACT) {
      ...
      for (RepositoryPlugin plugin : plugins) {
        ...
        if (result != null) return toContainer(bsn, range, attrs, result);
      }
    }
    else {
      ...
      for (RepositoryPlugin plugin : plugins) {
        try {
          ...
          if (vs != null) {
            for (Version v : vs) {
              if (!versions.containsKey(v) && versionRange.includes(v)) versions.put(v, plugin);
            }
          }
        }
          ...
          if (!versions.isEmpty() && Verifier.isVersion(range)) {
            ...
            if (file != null) return toContainer(bsn, range, attrs, file);
          }
        }
      }
      ...
      if (!versions.isEmpty()) {
        ...
        if (provider != null) {
          ...
          if (result != null) return toContainer(bsn, version, attrs, result);
        }
        else {
          error("Unexpected, found versions %s but then not provider for startegy %s?", versions, useStrategy);
        }
      }
    }
    ...
    for (File output : outputs) {
      ...
      try {
        for (Deploy d : getPlugins(Deploy.class)) {
          ...
          try {
            if (d.deploy(this, jar)) trace("deployed %s successfully to %s", output, d);
          }
          catch (Exception e) {
    ...
    if (args.length > 2) {
      ...
      if (args.length == 4) {
        if (args[3].equalsIgnoreCase("HIGHEST")) {
          ...
          error(
            "${repo;<bsn>;<version>;<'highest'|'lowest'|'exact'>} macro requires a strategy of 'highest' or 'lowest', and is " + args[3]);
        }
      }
    }
  ...
  public boolean isStale() throws Exception {
    ...
    for (File f : files) {
      if (f.lastModified() < lastModified()) return true;

      if (buildTime < f.lastModified()) buildTime = f.lastModified();
    }

    for (Project dependency : getDependson()) {
      if (dependency.isStale()) return true;

      if (dependency.isNoBundles()) continue;

      File[] deps = dependency.getBuildFiles();
      for (File f : deps) {
        if (f.lastModified() >= buildTime) return true;
      }
    }
    return false;
  }
  ...
  public File[] buildLocal(boolean underTest) throws Exception {
    ...
    if (underTest) builder.setProperty(Constants.UNDERTEST, "true");
  ...
  private void reportNewer(long lastModified, Jar jar) {
    if (isTrue(getProperty(Constants.REPORTNEWER))) {
      ...
      if (sb.length() > 0) warning(sb.toString());
    }
  }
  ...
  public Map<String, Action> getActions() {
    ...
    for (Map.Entry<String, Action> action : all.entrySet()) {
      ...
      if (key != null && key.trim().length() != 0) actions.put(key, action.getValue());
    }
    return actions;
  }
  ...
  public void fillActions(Map<String, Action> all) {
    ...
    Map<String, Map<String, String>> actions = parseHeader(getProperty("-actions", DEFAULT_ACTIONS));
  ...
  public void clean() throws Exception {
    ...
    if (getOutput().isDirectory()) IO.delete(getOutput());
    getOutput().mkdirs();
  }
  ...
  public void action(String command) throws Throwable {
    ...
    if (a == null) a = new ReflectAction(command);
  ...
  void tree(List<String> list, File current, String path, Instruction instr) {
    ...
    if (subs != null) {
      for (String sub : subs) {
        ...
        if (f.isFile()) {
          if (instr.matches(sub) && !instr.isNegated()) list.add(path + sub);
        }
        else {
  ...
  public String _help(String args[]) throws Exception {
    if (args.length == 1) return "Specify the option or header you want information for";

    Syntax syntax = Syntax.HELP.get(args[1]);
    if (syntax == null) return "No help for " + args[1];

    String what = null;
    if (args.length > 2) what = args[2];

    if (what == null || what.equals("lead")) return syntax.getLead();
    if (what == null || what.equals("example")) return syntax.getExample();
    if (what == null || what.equals("pattern")) return syntax.getPattern();
    if (what == null || what.equals("values")) return syntax.getValues();
  ...
  public Collection<Container> getDeliverables() throws Exception {
    ...
    for (Builder builder : builders) {
      Container c =
        new Container(this, builder.getBsn(), builder.getVersion(), Container.TYPE.PROJECT, getOutputFile(builder.getBsn()), null, null);
      result.add(c);
    }
    return result;
  ...
  public Builder getSubBuilder(File bndFile) throws Exception {
    ...
    if (!bndFile.getAbsolutePath().startsWith(base.getAbsolutePath())) return null;
  ...
  public Container getDeliverable(String bsn, Map<String, String> attrs) throws Exception {
    Collection<? extends Builder> builders = getSubBuilders();
    for (Builder sub : builders) {
      if (sub.getBsn().equals(bsn)) return new Container(this, getOutputFile(bsn));
    }
    return null;
  }
        ...
        if (launcher != null) {
          ...
          if (clz != null) {
            if (!target.isAssignableFrom(clz)) {
              error("Found a %s class in %s but it is not compatible with: %s", clz, c, target);
            }
            else {
  ...
  public synchronized boolean lock(String reason) throws InterruptedException {
    if (!lock.tryLock(5, TimeUnit.SECONDS)) {
      error("Could not acquire lock for %s, was locked by %s for %s", reason, lockingThread, lockingReason);
      System.out.printf("Could not acquire lock for %s, was locked by %s for %s\n", reason, lockingThread, lockingReason);
      System.out.flush();
      return false;
    }
