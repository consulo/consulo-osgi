...
class ReferenceDef {
  ...
  public void prepare(Analyzer analyzer) {
    ...
    if (version == null) version = AnnotationReader.V1_1;
  ...
  public Tag getTag() {
    ...
    if (cardinality != null) ref.addAttribute("cardinality", p(cardinality));
    if (policy != null) ref.addAttribute("policy", p(policy));

    if (interfce != null) ref.addAttribute("interface", interfce);

    if (target != null) ref.addAttribute("target", target);

    if (bind != null) ref.addAttribute("bind", bind);

    if (unbind != null) ref.addAttribute("unbind", unbind);
