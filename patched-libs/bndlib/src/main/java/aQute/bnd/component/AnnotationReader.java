...
public class AnnotationReader extends ClassDataCollector {
  ...
  final static Pattern PROPERTY_PATTERN = Pattern.compile("([^=]+(:(Boolean|Byte|Char|Short|Integer|Long|Float|Double|String))?)\\s*=(.*)");
  ...
  static Pattern ACTIVATEDESCRIPTOR =
    Pattern.compile("\\(((Lorg/osgi/service/component/ComponentContext;)|(Lorg/osgi/framework/BundleContext;)|(Ljava/util/Map;))*\\)V");
  ...
  private ComponentDef getDef(Clazz c, Analyzer analyzer) throws Exception {
    ...
    if (component.implementation == null) return null;

    while (extendsClass != null) {
      if (extendsClass.startsWith("java/")) break;

      Clazz ec = analyzer.findClass(extendsClass);
      if (ec == null) {
        analyzer.error("Missing super class for DS annotations: " + Clazz.pathToFqn(extendsClass) + " from " + c.getFQN());
      }
      else {
        c.parseClassFileWithCollector(this);
      }
    ...
    if (component.implementation != null) {
      for (ReferenceDef rdef : component.references.values()) {
        ...
        rdef.modified = referredMethod(analyzer, rdef, rdef.modified, "(add|set)(.*)", "modified$2", "(.*)", "modified$1");
      }
      return component;
    }
    else {
  ...
  protected String referredMethod(Analyzer analyzer, ReferenceDef rdef, String value, String... matches) {
    ...
    else if (value.equals("-")) return null;
  ...
  public void annotation(Annotation annotation) {
    try {
      ...
      else if (a instanceof Reference) doReference((Reference)a, annotation);
  ...
  protected void doReference(Reference reference, Annotation raw) throws Exception {
    ...
    if (def.interfce != null) {
      ...
      if (m.matches()) {
        ...
        throw new IllegalArgumentException("Cannot detect the type of a Component Reference from the descriptor: " + methodDescriptor);
      }
    }
    ...
    if (component.references.containsKey(def.name)) {
      analyzer
        .error("In component %s, multiple references with the same name: %s. Previous def: %s, this def: %s", component.implementation,
               component.references.get(def.name), def.interfce, "");
    }
    else {
  ...
  protected void doComponent(Component comp, Annotation annotation) throws Exception {
    ...
    if (component.implementation != null) return;
    ...
    if (annotation.get("enabled") != null) component.enabled = comp.enabled();
    if (annotation.get("factory") != null) component.factory = comp.factory();
    if (annotation.get("immediate") != null) component.immediate = comp.immediate();
    if (annotation.get("servicefactory") != null) component.servicefactory = comp.servicefactory();
    ...
    if (x == null) {
      ...
      if (interfaces != null) {
        ...
        for (int i = 0; i < interfaces.length; i++) {
          if (!interfaces[i].equals("scala/ScalaObject")) result.add(Clazz.internalToFqn(interfaces[i]));
        }
  ...
  public void method(int access, String name, String descriptor) {
    if (Modifier.isPrivate(access) || Modifier.isAbstract(access) || Modifier.isStatic(access)) return;
