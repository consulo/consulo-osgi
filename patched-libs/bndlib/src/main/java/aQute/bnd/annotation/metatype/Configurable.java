...
public class Configurable<T> {
  ...
  public static <T> T createConfigurable(Class<T> c, Map<?, ?> properties) {
    Object o = Proxy.newProxyInstance(c.getClassLoader(), new Class<?>[]{c}, new ConfigurableHandler(properties, c.getClassLoader()));
    return c.cast(o);
  }
  ...
  static class ConfigurableHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      if (ad != null && !ad.id().equals(Meta.NULL)) id = ad.id();

      Object o = properties.get(id);

      if (o == null) {
        if (ad != null) {
          if (ad.required()) throw new IllegalStateException("Attribute is required but not set " + method.getName());

          o = ad.deflt();
          if (o.equals(Meta.NULL)) o = null;
        }
      }
      if (o == null) {
    ...
    public Object convert(Type type, Object o) throws Exception {
      ...
      if (actualType.isAssignableFrom(resultType)) return o;

      if (resultType == boolean.class || resultType == Boolean.class) {
        if (actualType == boolean.class || actualType == Boolean.class) return o;
        ...
        if (Number.class.isAssignableFrom(actualType)) return ((Number)o).byteValue();
        ...
        if (Number.class.isAssignableFrom(actualType)) return ((Number)o).shortValue();
        resultType = Short.class;
      }
      else if (resultType == int.class) {
        if (Number.class.isAssignableFrom(actualType)) return ((Number)o).intValue();
        resultType = Integer.class;
      }
      else if (resultType == long.class) {
        if (Number.class.isAssignableFrom(actualType)) return ((Number)o).longValue();
        resultType = Long.class;
      }
      else if (resultType == float.class) {
        if (Number.class.isAssignableFrom(actualType)) return ((Number)o).floatValue();
        resultType = Float.class;
      }
      else if (resultType == double.class) {
        if (Number.class.isAssignableFrom(actualType)) return ((Number)o).doubleValue();
        resultType = Double.class;
      }

      if (resultType.isPrimitive()) throw new IllegalArgumentException("Unknown primitive: " + resultType);
    ...
    private Object convert(ParameterizedType pType, Object o) throws InstantiationException, IllegalAccessException, Exception {
      ...
      if (Collection.class.isAssignableFrom(resultType)) {
        ...
        @SuppressWarnings("unchecked") Collection<Object> result = (Collection<Object>)resultType.newInstance();
      ...
      throw new IllegalArgumentException("cannot convert to " + pType + " because it uses generics and is not a Collection");
    }
    ...
    private Collection<?> toCollection(Object o) {
      ...
      if (o instanceof String) {
        String s = (String)o;
        if (s.indexOf('|') > 0) return Arrays.asList(s.split("\\|"));
      }
