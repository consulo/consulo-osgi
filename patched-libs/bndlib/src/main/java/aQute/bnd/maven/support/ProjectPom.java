...
public class ProjectPom extends Pom {
  ...
  protected void parse(Document doc, XPath xp) throws Exception {
    ...
    if (parent != null && parent.hasChildNodes()) {
      ...
      if (parentFile.isFile()) {
        ...
        for (Enumeration<?> e = parentPom.properties.propertyNames(); e.hasMoreElements(); ) {
          ...
          if (!properties.contains(key)) properties.put(key, parentPom.properties.get(key));
        }
    ...
    for (int i = 0; i < propNodes.getLength(); i++) {
      ...
      if (key == null || key.length() == 0) throw new IllegalArgumentException("Pom has an empty or null key");
      if (value == null || value.length() == 0) throw new IllegalArgumentException("Pom has an empty or null value for property " + key);
    ...
    for (int i = 0; i < repos.getLength(); i++) {
      ...
      if (uri.getScheme() == null) uri = IO.getFile(pomFile.getParentFile(), URIString).toURI();
...
//	}
  ...
  private void setNames(Pom pom) throws Exception {
    if (artifactId == null || artifactId.length() == 0) artifactId = pom.getArtifactId();
    if (groupId == null || groupId.length() == 0) groupId = pom.getGroupId();
    if (version == null || version.length() == 0) version = pom.getVersion();
    if (description == null) {
  ...
  private String get(String key) {
    if (key.equals("pom.artifactId")) return artifactId;
    if (key.equals("pom.groupId")) return groupId;
    if (key.equals("pom.version")) return version;

    if (key.equals("pom.name")) return name;
