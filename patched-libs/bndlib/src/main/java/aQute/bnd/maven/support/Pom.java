...
public abstract class Pom {
  ...
  public class Dependency {
    ...
    public String toString() {
      ...
      if (groupId != null) builder.append("groupId=").append(groupId).append(", ");
      if (artifactId != null) builder.append("artifactId=").append(artifactId).append(", ");
      if (version != null) builder.append("version=").append(version).append(", ");
      if (type != null) builder.append("type=").append(type).append(", ");
      if (scope != null) builder.append("scope=").append(scope).append(", ");
      builder.append("optional=").append(optional).append(", ");
      if (exclusions != null) builder.append("exclusions=").append(exclusions);
      builder.append("]");
  ...
  protected void parse(Document doc, XPath xp) throws XPathExpressionException, Exception {
    ...
    if (this.description.length() != 0 && nextDescription.length() != 0) this.description += "\n";
  ...
  private String replace(String key, String dflt) {
    if (key == null || key.length() == 0) return dflt;
  ...
  public String getVersion() throws Exception {
    if (version == null) return "<not set>";
    return replace(version);
  }
  ...
  public Set<Pom> getDependencies(Scope scope, URI... urls) throws Exception {
    ...
    while (!queue.isEmpty()) {
      ...
      if (rover.excludes(name) || dep.optional) continue;
      ...
      if (dep.scope == scope && !dep.optional) {
        try {
          ...
          if (sub != null) {
            ...
            System.out.println("Cannot find " + dep + " from " + rover.previous.dependency);
          }
          else {
...
//					}
  ...
  protected String replace(String in) {
    ...
    in = in.trim();
    if ("${pom.version}".equals(in) || "${version}".equals(in) || "${project.version}".equals(in)) return version;

    if ("${basedir}".equals(in)) return pomFile.getParentFile().getAbsolutePath();

    if ("${pom.name}".equals(in) || "${project.name}".equals(in)) return name;

    if ("${pom.artifactId}".equals(in) || "${project.artifactId}".equals(in)) return artifactId;
    if ("${pom.groupId}".equals(in) || "${project.groupId}".equals(in)) return groupId;
  ...
  public File getLibrary(Scope action, URI... repositories) throws Exception {
    ...
    if (file.isFile() && file.lastModified() >= getPomFile().lastModified()) return file;
