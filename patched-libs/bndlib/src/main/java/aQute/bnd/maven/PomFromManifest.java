...
public class PomFromManifest extends WriteResource {
  ...
  public String getBsn() {
    if (xbsn == null) xbsn = manifest.getMainAttributes().getValue(Constants.BUNDLE_SYMBOLICNAME);
    if (xbsn == null) throw new RuntimeException("Cannot create POM unless bsn is set");
    ...
    if (xgroupId == null) xgroupId = xbsn.substring(0, n);
    if (xartifactId == null) {
      xartifactId = xbsn.substring(n + 1);
      n = xartifactId.indexOf(';');
      if (n > 0) xartifactId = xartifactId.substring(0, n).trim();
    }
  ...
  public Version getVersion() {
    if (xversion != null) return new Version(xversion);
  ...
  public void write(OutputStream out) throws IOException {
    ...
    if (scmheader != null) scm.add(scmheader);
    ...
    if (!developers.isEmpty()) {
      ...
      for (String email : developers) {
        ...
        if (organization != null) new Tag(developer, "organization").addContent(organization);
      }
    }
    if (licenses != null) {
