...
public class CalltreeResource extends WriteResource {
  ...
  private static void xref(Map<Clazz.MethodDef, Set<Clazz.MethodDef>> references, Clazz.MethodDef source, Clazz.MethodDef reference) {
    ...
    if (set == null) references.put(source, set = new TreeSet<Clazz.MethodDef>());
  ...
  private static void method(PrintWriter out, String element, Clazz.MethodDef source, String closeElement) {
    out.println("      <" +
                ...
                (source.isConstructor() ? "" : " name='" + source.name + "'") +
                " descriptor='" +
                source.descriptor +
                "' pretty='" +
                source.getPretty() +
  ...
  private static String getAccess(int access) {
    ...
    if (Modifier.isPublic(access)) sb.append(" public='true'");
    if (Modifier.isStatic(access)) sb.append(" static='true'");
    if (Modifier.isProtected(access)) sb.append(" protected='true'");
    if (Modifier.isInterface(access)) sb.append(" interface='true'");
