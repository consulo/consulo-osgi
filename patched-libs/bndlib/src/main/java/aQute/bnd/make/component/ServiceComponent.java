...
public class ServiceComponent implements AnalyzerPlugin {
  ...
  private static class ComponentMaker extends Processor {
     ...
     * @throws IOException
     */
    private void componentEntry(Map<String, Map<String, String>> serviceComponents, String name, Map<String, String> info)
      ...
      if (annotations) {
        ...
        Collection<Clazz> annotatedComponents = analyzer.getClasses("", QUERY.ANNOTATION.toString(), Component.class.getName(), //
    ...
    private Collection<Clazz> checkAnnotationsFeasible(String name) throws Exception {
      ...
      for (Clazz c : not) {
        if (c.getFormat().hasAnnotations()) return not;
      }
    ...
    void annotated(Map<String, Map<String, String>> components, Clazz c, Map<String, String> info) throws Exception {
      ...
      if (localname == null) localname = c.getFQN();
      ...
      if (merged != null && merged.length() > 0) map.put(COMPONENT_PROPERTIES, merged);
      map.putAll(info);
      createComponentResource(components, localname, map);
    }

    private void createComponentResource(Map<String, Map<String, String>> components, String name, Map<String, String> info)
      throws IOException {

      // We can override the name in the parameters
      if (info.containsKey(COMPONENT_NAME)) name = info.get(COMPONENT_NAME);
      ...
      boolean designate =
        designate(name, info.get(COMPONENT_DESIGNATE), false) || designate(name, info.get(COMPONENT_DESIGNATEFACTORY), true);

      // If we had a designate, we want a default configuration policy of
      // require.
      if (designate && info.get(COMPONENT_CONFIGURATION_POLICY) == null) info.put(COMPONENT_CONFIGURATION_POLICY, "require");
    ...
    Resource createComponentResource(String name, String impl, Map<String, String> info) throws IOException {
      ...
      doAttribute(pw, info.get(COMPONENT_CONFIGURATION_POLICY), "configuration-policy", "optional", "require", "ignore");
    ...
    void reference(Map<String, String> info, PrintWriter pw) {
      ...
      for (Map.Entry<String, String> entry : info.entrySet()) {
        ...
        if (descriptors.size() > 0) {
          ...
          if (!descriptors.contains(bind)) error("The bind method %s for %s not defined", bind, referenceName);
