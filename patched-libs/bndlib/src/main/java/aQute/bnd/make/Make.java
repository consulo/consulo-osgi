...
public class Make {
      ...
      if (m.matches() || instr.isNegated()) {
        ...
        for (MakePlugin plugin : plugins) {
          try {
            ...
            builder.error("Plugin " + plugin + " generates error when use in making " + source + " with args " + arguments, e);
          }
        }
      }
    }
  ...
  Map<Instruction, Map<String, String>> getMakeHeader() {
    ...
    for (Iterator<Map.Entry<String, Map<String, String>>> e = make.entrySet().iterator(); e.hasNext(); ) {
