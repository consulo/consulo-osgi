...
public class Signatures {
  ...
  class Rover {
    ...
    char take(char c) {
      char x = s.charAt(i++);
      if (c != x) throw new IllegalStateException("get() expected " + c + " but got + " + x);
      return x;
    }

    public String upTo(String except) {
      int start = i;
      while (except.indexOf(peek()) < 0) take();
  ...
  public String getSignature(Object c) throws Exception {
    if (c instanceof Class<?>) return getSignature((Class<?>)c);
    if (c instanceof Constructor<?>) return getSignature((Constructor<?>)c);
    if (c instanceof Method) return getSignature((Method)c);
    if (c instanceof Field) return getSignature((Field)c);
  ...
  private boolean isInterface(Object type) throws Exception {
    if (type instanceof Class) return (((Class<?>)type).isInterface());

    if (isInstance(type.getClass(), "java.lang.reflect.ParameterizedType")) return isInterface(call(type, "getRawType"));
  ...
  private void reference(StringBuffer sb, Object t) throws Exception {
    ...
    if (isInstance(t.getClass(), "java.lang.reflect.ParameterizedType")) {
      ...
      if (upperBounds.length == 1 && upperBounds[0] == Object.class) upperBounds = new Object[0];
   ...
   */
  private void reference(StringBuffer sb, Map<String, String> map, Rover rover, boolean primitivesAllowed) {
    ...
    if (type == '[') {
      ...
      if (!primitivesAllowed) throw new IllegalStateException("Primitives are not allowed without an array");
    }
  }
  ...
  private boolean isInstance(Class<?> type, String string) {
    ...
    if (type.getName().equals(string)) return true;

    if (isInstance(type.getSuperclass(), string)) return true;

    for (Class<?> intf : type.getInterfaces()) {
      if (isInstance(intf, string)) return true;
    }
    return false;
  }
