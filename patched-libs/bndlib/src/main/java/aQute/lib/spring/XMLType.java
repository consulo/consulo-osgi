...
public class XMLType {
  ...
  public boolean analyzeJar(Analyzer analyzer) throws Exception {
    ...
    if (dir == null || dir.isEmpty()) {
      ...
      if (resource != null) process(analyzer, root, resource);
      return false;
    }
  ...
  private void process(Analyzer analyzer, String path, Resource resource) {
    try {
      ...
      for (Iterator<String> r = set.iterator(); r.hasNext(); ) {
        String pack = r.next();
        if (!QN.matcher(pack).matches()) analyzer.warning("Package does not seem a package in spring resource (" + path + "): " + pack);
        if (!analyzer.getReferred().containsKey(pack)) analyzer.getReferred().put(pack, new LinkedHashMap<String, String>());
      }
    }
    catch (Exception e) {
