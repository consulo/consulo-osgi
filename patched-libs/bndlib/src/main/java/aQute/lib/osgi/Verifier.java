...
public class Verifier extends Analyzer {
  ...
  final static EE[] ees =
    {new EE("CDC-1.0/Foundation-1.0", V1_3, V1_1), new EE("CDC-1.1/Foundation-1.1", V1_3, V1_2), new EE("OSGi/Minimum-1.0", V1_3, V1_1),
      new EE("OSGi/Minimum-1.1", V1_3, V1_2), new EE("JRE-1.1", V1_1, V1_1), //
      new EE("J2SE-1.2", V1_2, V1_1), //
      new EE("J2SE-1.3", V1_3, V1_1), //
      new EE("J2SE-1.4", V1_3, V1_2), //
      new EE("J2SE-1.5", V1_5, V1_5), //
      new EE("JavaSE-1.6", V1_6, V1_6), new EE("PersonalJava-1.1", V1_1, V1_1), new EE("PersonalJava-1.2", V1_1, V1_1),
      new EE("CDC-1.0/PersonalBasis-1.0", V1_3, V1_1), new EE("CDC-1.0/PersonalJava-1.0", V1_3, V1_1),
      new EE("CDC-1.1/PersonalBasis-1.1", V1_3, V1_2), new EE("CDC-1.1/PersonalJava-1.1", V1_3, V1_2)};
  ...
  public final static Pattern VERSIONRANGE =
    Pattern.compile("((\\(|\\[)" + VERSION_STRING + "," + VERSION_STRING + "(\\]|\\)))|" + VERSION_STRING);
  final static Pattern FILE = Pattern.compile("/?[^/\"\n\r\u0000]+(/[^/\"\n\r\u0000]+)*");
  final static Pattern WILDCARDPACKAGE = Pattern.compile("((\\p{Alnum}|_)+(\\.(\\p{Alnum}|_)+)*(\\.\\*)?)|\\*");
  ...
  public final static String EES[] =
    {"CDC-1.0/Foundation-1.0", "CDC-1.1/Foundation-1.1", "OSGi/Minimum-1.0", "OSGi/Minimum-1.1", "OSGi/Minimum-1.2", "JRE-1.1", "J2SE-1.2",
      "J2SE-1.3", "J2SE-1.4", "J2SE-1.5", "JavaSE-1.6", "JavaSE-1.7", "PersonalJava-1.1", "PersonalJava-1.2", "CDC-1.0/PersonalBasis-1.0",
      "CDC-1.0/PersonalJava-1.0"};
  ...
  private void verifyHeaders(Attributes main) {
    for (Object element : main.keySet()) {
      ...
      if (!HEADER_PATTERN.matcher(h).matches()) error("Invalid Manifest header: " + h + ", pattern=" + HEADER_PATTERN);
    }
  }
          ...
          if (resource != null) {
            ...
            try {
              ...
              for (Map.Entry<String, Resource> entry : dot.getResources().entrySet()) {
                if (entry.getKey().startsWith(jarOrDir)) sub.putResource(entry.getKey().substring(jarOrDir.length() + 1), entry.getValue());
              }
              list.add(sub);
            }
            catch (Exception e) {
    ...
    if (bactivator != null) {
      ...
      if (cl == null) {
        ...
        if (n > 0) {
          ...
          if (mimports.containsKey(pack)) return;
          error("Bundle-Activator not found on the bundle class path nor in imports: " + bactivator);
        }
        else {
          error("Activator uses default package and is not local (default package can not be imported): " + bactivator);
        }
      }
    }
  }
  ...
  private void verifyInvalidExports() {
    ...
    if (!invalidExport.isEmpty()) error("Exporting packages that are not on the Bundle-Classpath" + bundleClasspath + ": " + invalidExport);
  }
  ...
  private void verifyInvalidImports() {
    ...
    if (isPedantic() && !invalidImport.isEmpty()) {
      warning("Importing packages that are never refered to by any class on the Bundle-Classpath" + bundleClasspath + ": " + invalidImport);
    }
  }
  ...
  private void verifyUnresolvedReferences() {
    ...
    if (!unresolvedReferences.isEmpty()) {
      ...
      for (Clazz clazz : classSpace.values()) {
        if (hasOverlap(unresolvedReferences, clazz.getReferred())) culprits.add(clazz.getPath());
      }

      error(
        "Unresolved references to " + unresolvedReferences + " by class(es) on the Bundle-Classpath" + bundleClasspath + ": " + culprits);
    }
  }
  ...
  private boolean isDynamicImport(String pack) {
    for (String pattern : mdynimports.keySet()) {
      // Wildcard?
      if (pattern.equals("*")) return true; // All packages can be dynamically imported

      if (pattern.endsWith(".*")) {
        pattern = pattern.substring(0, pattern.length() - 2);
        if (pack.startsWith(pattern) && (pack.length() == pattern.length() || pack.charAt(pattern.length()) == '.')) return true;
      }
      else {
        if (pack.equals(pattern)) return true;
      }
    }
    return false;
  }
  ...
  private boolean hasOverlap(Set<?> a, Set<?> b) {
    for (Iterator<?> i = a.iterator(); i.hasNext(); ) {
      if (b.contains(i.next())) return true;
    }
    return false;
  }
  ...
  public void verify() throws Exception {
    if (classSpace == null) {
      classSpace = analyzeBundleClasspath(dot, parseHeader(getHeader(Analyzer.BUNDLE_CLASSPATH)), contained, referred, uses);
    }
  ...
  public void verifyBundleClasspath() {
    ...
    if (bcp.isEmpty() || bcp.containsKey(".")) return;

    for (String path : bcp.keySet()) {
      if (path.endsWith("/")) error("A Bundle-ClassPath entry must not end with '/': %s", path);
    ...
    for (String path : dot.getResources().keySet()) {
      if (path.endsWith(".class")) {
        warning(
          "The Bundle-Classpath does not contain the actual bundle JAR (as specified with '.' in the Bundle-Classpath) but the JAR does contain classes. Is this intentional?");
        return;
      }
    }
  }
  ...
  private void verifySymbolicName() {
    ...
    if (!bsn.isEmpty()) {
      if (bsn.size() > 1) error("More than one BSN specified " + bsn);
        ...
        case '\\':
          if ("\\)(*".indexOf(expr.charAt(index + 1)) >= 0) {
            ...
            throw new IllegalArgumentException(
              "Filter error, illegal use of backslash at index " + index + ". Backslash may only be used before * or () or \\");
          }
      }
      sb.append(expr.charAt(index++));
    }
  ...
  public String getProperty(String key, String deflt) {
    if (properties == null) return deflt;
  ...
  public static boolean isMember(String value, String[] matches) {
    for (String match : matches) {
      if (match.equals(value)) return true;
    }
    return false;
  }
