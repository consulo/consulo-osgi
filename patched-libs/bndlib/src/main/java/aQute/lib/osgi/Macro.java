...
public class Macro implements Replacer {
  ...
  public String _if(String args[]) {
    ...
    if (condition.length() != 0) return args[2];
    if (args.length > 3) {
      return args[3];
    }
    else {
  ...
  public String _literal(String args[]) {
    if (args.length != 2) throw new RuntimeException("Need a value for the ${literal;<value>} macro");
    return "${" + args[1] + "}";
  }
  ...
  public String _toclasspath(String args[]) {
    ...
    boolean cl = true;
    if (args.length > 2) cl = new Boolean(args[2]);
  ...
  final static String _versionHelp =
  ...
  String version(Version version, String mask) {
    if (version == null) {
      ...
      if (v == null) {
        domain.error("No version specified for ${version} or ${range} and no implicit version ${@} either, mask=%s", mask);
        v = "0";
      }
  ...
  public String system_internal(boolean allowFail, String args[]) throws Exception {
    verifyCommand(args, "${" + (allowFail ? "system-allow-fail" : "system") + ";<command>[;<in>]}, execute a system command", null, 2, 3);
    ...
    if (args.length > high) {
      ...
      for (int i = 0; patterns != null && i < patterns.length && i < args.length; i++) {
        if (patterns[i] != null) {
          ...
          if (!m.matches()) message += String.format("Argument %s (%s) does not match %s\n", i, args[i], patterns[i].pattern());
        }
      }
    }
    if (message.length() != 0) {
