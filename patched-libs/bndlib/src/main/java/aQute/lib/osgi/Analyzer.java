...
public class Analyzer extends Processor {
    ...
    if (!analyzed) {
      ...
      for (Iterator<String> i = extra.iterator(); i.hasNext(); ) {
        ...
        if (p.startsWith("!") || p.indexOf('*') >= 0 || p.indexOf('?') >= 0 || p.indexOf('[') >= 0) {
          if (!isResourceOnly() && !(p.equals("*"))) warning("Did not find matching referal for " + p);
        }
        else {
  ...
  Set<String> getUsedBy(String pack) {
    ...
    for (Iterator<Map.Entry<String, Set<String>>> i = uses.entrySet().iterator(); i.hasNext(); ) {
      ...
      if (used.contains(pack)) set.add(entry.getKey());
    }
    return set;
  }
  ...
  public Manifest calcManifest() throws Exception {
    ...
    while (i.hasNext()) if (toBeRemoved.contains(i.next())) i.remove();
  ...
  private void doNamesection(Jar dot, Manifest manifest) {
    ...
    Map<String, Map<String, String>> namesection = parseHeader(getProperties().getProperty(NAMESECTION));
    Set<Entry<Instruction, Map<String, String>>> instructions = Instruction.replaceWithInstruction(namesection).entrySet();
    Set<Map.Entry<String, Resource>> resources = new HashSet<Map.Entry<String, Resource>>(dot.getResources().entrySet());
    ...
    for (Map.Entry<Instruction, Map<String, String>> instr : instructions) {
      ...
      if (!matched) warning("The instruction %s in %s did not match any resources", instr.getKey(), NAMESECTION);
    }
  ...
  public String getBsn() {
    ...
    if (value == null) {
      if (getPropertiesFile() != null) value = getPropertiesFile().getName();
      ...
      if (value == null || value.equals("bnd.bnd")) {
        ...
        if (!value.startsWith(getBase().getName())) value = projectName + "." + value;
      }
    }
    ...
    int n = value.indexOf(';');
    if (n > 0) value = value.substring(0, n);
    return value.trim();
  }
  ...
  public boolean checkClass(String interfaceName) {
    ...
    if (interfaceName.startsWith("java.")) return true;
    ...
    if (imports != null && !imports.isEmpty()) {
      ...
      if (imports.containsKey(pack)) return true;
    }
    int n = interfaceName.lastIndexOf('.');
    if (n > 0 && n + 1 < interfaceName.length() && Character.isUpperCase(interfaceName.charAt(n + 1))) {
  ...
  private String fileName(String path) {
    int n = path.lastIndexOf('/');
    if (n > 0) return path.substring(n + 1);
    return path;
  }
  ...
  void merge(Manifest result, Manifest old) throws IOException {
    if (old != null) {
      for (Iterator<Map.Entry<Object, Object>> e = old.getMainAttributes().entrySet().iterator(); e.hasNext(); ) {
        ...
        if (name.toString().equalsIgnoreCase("Created-By")) name = new Attributes.Name("Originally-Created-By");
        if (!result.getMainAttributes().containsKey(name)) result.getMainAttributes().put(name, value);
      }
  ...
  Map<String, Map<String, String>> doExportsToImports(Map<String, Map<String, String>> exports) {
    ...
    for (String p : privatePackages) {
      ...
      if (uses != null) privateReferences.addAll(uses);
    }
    ...
    if (imports != null) toBeImported.removeAll(imports.keySet());
  ...
  private <T> boolean intersects(Collection<T> aa, Collection<T> bb) {
    ...
    if (aa.size() > bb.size()) return intersects(bb, aa);

    for (T t : aa) {
      if (bb.contains(t)) return true;
    }
    return false;
  }
  ...
  public boolean referred(String packageName) {
    ...
    for (Map.Entry<String, Set<String>> contained : uses.entrySet()) {
      if (!contained.getKey().equals(packageName)) {
        if (contained.getValue().contains(packageName)) return true;
      }
    }
    return false;
  }
      ...
      try {
        ...
        if (exporterAttributes != null) {
          ...
          if (exporterAttributes.containsKey(IMPORT_DIRECTIVE)) {
            importAttributes.put(IMPORT_DIRECTIVE, exporterAttributes.get(IMPORT_DIRECTIVE));
          }
        }
    ...
    for (String packageName : exports.keySet()) {
      ...
      try {
        ...
        for (Map.Entry<String, String> entry : exporterAttributes.entrySet()) {
          ...
          if (key.equalsIgnoreCase(SPECIFICATION_VERSION)) key = VERSION_ATTRIBUTE;
  ...
  private void augmentMandatory(Map<String, String> currentAttributes, Map<String, String> exporter) {
    ...
    if (mandatory != null) {
      ...
      for (int i = 0; i < attrs.length; i++) {
        if (!currentAttributes.containsKey(attrs[i])) currentAttributes.put(attrs[i], exporter.get(attrs[i]));
      }
    }
  }
  ...
  String findPath(String name, String[] args, boolean fullPathName) {
    if (args.length > 3) {
      warning(
        "Invalid nr of arguments to " + name + " " + Arrays.asList(args) + ", syntax: ${" + name + " (; reg-expr (; replacement)? )? }");
      return null;
    }
  ...
  public void putAll(Map<String, String> additional, boolean force) {
    for (Iterator<Map.Entry<String, String>> i = additional.entrySet().iterator(); i.hasNext(); ) {
      Map.Entry<String, String> entry = i.next();
      if (force || getProperties().get(entry.getKey()) == null) setProperty((String)entry.getKey(), (String)entry.getValue());
    }
  }
  ...
  public void addClasspath(Jar jar) {
    if (isPedantic() && jar.getResources().isEmpty()) warning("There is an empty jar or directory on the classpath: " + jar.getName());

    classpath.add(jar);
  }

  public void addClasspath(File cp) throws IOException {
    if (!cp.exists()) warning("File on classpath that does not exist: " + cp);
    Jar jar = new Jar(cp);
    addClose(jar);
    classpath.add(jar);
  }
          ...
          if (dot.getDirectories().containsKey(path)) {
            ...
            if (bundleClasspath.containsKey(".")) {
              warning("Bundle-ClassPath uses a directory '%s' as well as '.', this implies the directory is seen \n" +
                      ...
                      "loaded from '%s'. It is better to unroll the directory to create a flat bundle.", path, path);
            }
            analyzeJar(dot, Processor.appendPath(path) + "/", classSpace, contained, referred, uses, hide, true);
          }
          else {
            if (!"optional".equals(info.get(RESOLUTION_DIRECTIVE))) warning("No sub JAR or directory " + path);
          }
        }
      }
  ...
  private void analyzeJar(Jar jar,
                          ...
                          Map<String, Set<String>> uses,
                          Set<String> hide,
                          boolean reportWrongPath) throws Exception {
        ...
        if (pack != null && !contained.containsKey(pack)) {
          ...
          if (!isMetaData(relativePath)) {
            ...
            if (pinfo != null) {
              ...
              if (version != null) info.put(VERSION_ATTRIBUTE, version);
            }
          }
        }
    ...
    clazz.parseClassFileWithCollector(new ClassDataCollector() {
      ...
      public void annotation(Annotation a) {
        if (a.name.equals(Clazz.toDescriptor(aQute.bnd.annotation.Version.class))) {
          ...
          if (!info.containsKey(Constants.VERSION_ATTRIBUTE)) {
            if (version != null) {
              ...
              if (Verifier.VERSION.matcher(version).matches()) {
                ...
                error("Export annotatio in %s has invalid version info: %s", clazz, version);
              }
            }
          }
          else {
          ...
          Map<String, String> attrs = doAttrbutes((Object[])a.get(Export.MANDATORY), clazz, getReplacer());
          if (!attrs.isEmpty()) {
            info.putAll(attrs);
   ...
   * @return
   */
  static Pattern fuzzyVersion = Pattern.compile("(\\d+)(\\.(\\d+)(\\.(\\d+))?)?([^a-zA-Z0-9](.*))?", Pattern.DOTALL);
  static Pattern fuzzyVersionRange = Pattern.compile("(\\(|\\[)\\s*([-\\da-zA-Z.]+)\\s*,\\s*([-\\da-zA-Z.]+)\\s*(\\]|\\))", Pattern.DOTALL);
  ...
  private static String removeLeadingZeroes(String group) {
    int n = 0;
    while (group != null && n < group.length() - 1 && group.charAt(n) == '0') n++;
    if (n == 0) return group;
  ...
  static void cleanupModifier(StringBuffer result, String modifier) {
    ...
    if (m.matches()) modifier = m.group(2);
  ...
  boolean isMetaData(String pack) {
    for (int i = 0; i < METAPACKAGES.length; i++) {
      if (pack.startsWith(METAPACKAGES[i])) return true;
    }
    return false;
  }
  ...
  static String _classesHelp =
    "${classes;'implementing'|'extending'|'importing'|'named'|'version'|'any';<pattern>}, Return a list of class fully qualified class names that extend/implement/import any of the contained classes matching the pattern\n";
  ...
  public Collection<Clazz> getClasses(String... args) throws Exception {
    ...
    for (int i = 1; i < args.length; i++) {
      ...
      else if (typeName.equalsIgnoreCase("implementing")) typeName = "implements";
  ...
  public void referTo(String impl) {
    ...
    if (!referred.containsKey(pack)) referred.put(pack, new LinkedHashMap<String, String>());
  }
  ...
  public String _md5(String args[]) throws Exception {
    ...
    if (r == null) throw new FileNotFoundException("From " + digester + ", not found " + args[1]);
  ...
  public String _sha1(String args[]) throws Exception {
    ...
    Resource r = dot.getResource(args[1]);
    if (r == null) throw new FileNotFoundException("From sha1, not found " + args[1]);
