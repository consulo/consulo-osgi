...
public class Builder extends Analyzer {
  ...
  public Jar build() throws Exception {
    init();
    if (isTrue(getProperty(NOBUNDLES))) return null;
    ...
    if (getProperty(POM) != null) dot.putResource("pom.xml", new PomResource(dot.getManifest()));

    if (!isNoBundle()) doVerify(dot);

    if (dot.getResources().isEmpty()) error("The JAR is empty: " + dot.getName());
  ...
  private Jar doWab(Jar dot) throws Exception {
    ...
    if (wab == null && wablib == null) return dot;
      ...
      if (cp != null) {
        ...
        for (String part : parts) {
          ...
          if (!sub.exists() || !sub.getParentFile().equals(f.getParentFile())) {
            warning("Invalid Class-Path entry %s in %s, must exist and must reside in same directory", sub, f);
          }
          else {
            addWabLib(dot, sub);
          }
  ...
  private void doConditional(Jar dot) throws Exception {
    ...
    if (conditionals.isEmpty()) return;
    ...
    while (true) {
      ...
      for (Map.Entry<String, Map<String, String>> entry : getImports().entrySet()) {
        ...
        if (type != null && type.equals(PRIVATE_DIRECTIVE)) filtered.put(entry.getKey(), entry.getValue());
      }
  ...
  public void cleanupVersion(Map<String, Map<String, String>> mapOfMap, String defaultVersion) {
    for (Iterator<Map.Entry<String, Map<String, String>>> e = mapOfMap.entrySet().iterator(); e.hasNext(); ) {
      ...
      if (v == null && defaultVersion != null) {
        if (!isTrue(getProperty(Constants.NODEFAULTVERSION))) {
          v = defaultVersion;
          if (isPedantic()) warning("Used bundle version %s for exported package %s", v, entry.getKey());
        }
        else {
          if (isPedantic()) warning("No export version for exported package %s", entry.getKey());
        }
      }
      if (v != null) attributes.put(Constants.VERSION_ATTRIBUTE, cleanupVersion(v));
    }
  }
    ...
    for (Iterator<String> cpe = classspace.keySet().iterator(); cpe.hasNext(); ) {
      ...
      for (Iterator<File> i = getSourcePath().iterator(); i.hasNext(); ) {
        ...
        if (f.exists()) {
          ...
          if (path.trim().length() == 0) System.out.println("Duh?");
  ...
  private void doExpand(Jar jar) throws IOException {
    ...
    Map<Instruction, Map<String, String>> privateMap = Instruction.replaceWithInstruction(getHeader(PRIVATE_PACKAGE));
    Map<Instruction, Map<String, String>> exportMap = Instruction.replaceWithInstruction(getHeader(EXPORT_PACKAGE));

    if (isTrue(getProperty(Constants.UNDERTEST))) {
      privateMap.putAll(Instruction.replaceWithInstruction(parseHeader(getProperty(Constants.TESTPACKAGES, "test;presence:=optional"))));
    }
    if (!privateMap.isEmpty()) doExpand(jar, "Private-Package, or -testpackages", privateMap, true);
    ...
    if (!isNoBundle()) {
      if (privateMap.isEmpty() && exportMap.isEmpty() && !isResourceOnly() && getProperty(EXPORT_CONTENTS) == null) {
        warning(
          "None of Export-Package, Provide-Package, Private-Package, -testpackages, or -exportcontents is set, therefore no packages will be included");
      }
    }
  }
  ...
  private void doExpand(Jar jar, String name, Map<Instruction, Map<String, String>> instructions, boolean mandatory) {
    ...
    if (mandatory && superfluous.size() > 0) {
      ...
      if (isPedantic()) diagnostics = true;
    }
  }
  ...
  private void doExpand(Jar jar, Map<Instruction, Map<String, String>> included, Jar classpathEntry, Set<Instruction> superfluous) {
    ...
    loop:
    for (Map.Entry<String, Map<String, Resource>> directory : classpathEntry.getDirectories().entrySet()) {
      String path = directory.getKey();

      if (doNotCopy(getName(path))) continue;
  ...
  private int getSplitStrategy(String type) {
    if (type == null) return SPLIT_DEFAULT;

    if (type.equals("merge-last")) return SPLIT_MERGE_LAST;

    if (type.equals("merge-first")) return SPLIT_MERGE_FIRST;

    if (type.equals("error")) return SPLIT_ERROR;

    if (type.equals("first")) return SPLIT_FIRST;
   ...
   * @return
   */
  private Instruction matches(Map<Instruction, Map<String, String>> instructions,
                              String pack,
    ...
    for (Map.Entry<Instruction, Map<String, String>> entry : instructions.entrySet()) {
      ...
      if (from != null) {
        ...
        if (!f.matches(source) || f.isNegated()) return null;
      }

      // Now do the normal
      // matching
      if (pattern.matches(pack)) {
        if (superfluousPatterns != null) superfluousPatterns.remove(pattern);
        return pattern;
      }
    }
    return null;
  ...
  private String doResourceDirectory(Jar jar, Map<String, String> extra, boolean preprocess, File sourceFile, String destinationPath)
    throws Exception {
  ...
  private void resolveFiles(File dir, FileFilter filter, boolean recursive, String path, Map<String, File> files, boolean flatten) {
    ...
    for (File file : fs) {
      if (file.isDirectory()) {
        ...
        if (files.containsKey(p)) warning("Include-Resource overwrites entry %s from file %s", p, file);
        files.put(p, file);
      }
    }
  }
  ...
  private void noSuchFile(Jar jar, String clause, Map<String, String> extra, String source, String destinationPath) throws Exception {
    ...
    if (src != null) {
      ...
      if (lastChance != null) {
        ...
        if (x != null) lastChance.setExtra(x);
  ...
  private void copy(Jar jar, String path, File from, boolean preprocess, Map<String, String> extra) throws Exception {
    if (doNotCopy(from.getName())) return;
    ...
    if (from.isDirectory()) {
      ...
      if (from.exists()) {
        ...
        String x = extra.get("extra");
        if (x != null) resource.setExtra(x);
        if (path.endsWith("/")) path = path + from.getName();
  ...
  public void addSourcepath(File cp) {
    if (!cp.exists()) warning("File on sourcepath that does not exist: " + cp);
  ...
  public Jar[] builds() throws Exception {
    ...
    for (Builder builder : builders) {
      ...
      if (builder != this) getInfo(builder, builder.getBsn() + ": ");
    }
  ...
  public List<Builder> getSubBuilders() throws Exception {
    ...
    if (sub == null || sub.trim().length() == 0 || EMPTY_HEADER.equals(sub)) return Arrays.asList(this);
    ...
    while (members.size() > 0) {
      ...
      Processor p = this;
      while (p != null) {
        if (file.equals(p.getPropertiesFile())) continue nextFile;
        p = p.getParent();
      }
