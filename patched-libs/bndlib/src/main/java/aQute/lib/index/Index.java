...
public class Index implements Iterable<byte[]> {
  ...
  class Page {
    ...
    void copy(ByteBuffer src, int srcPos, ByteBuffer dst, int dstPos, int length) {
      if (srcPos < dstPos) {
        while (length-- > 0) dst.put(dstPos + length, src.get(srcPos + length));

      }
      else {
        while (length-- > 0) dst.put(dstPos++, src.get(srcPos++));
      }
    }
    ...
    void insertNonFull(byte[] k, long v) throws IOException {
      ...
      if (leaf) {
        if (cmp != 0) {
          i++;
          if (i != n) copy(buffer, pos(i), buffer, pos(i + 1), size(n - i));
        }
        set(i, k, v);
        n++;
        dirty = true;
  ...
  public Index(File file, int keySize) throws IOException {
    ...
    if (this.file.size() == pageSize) {
      ...
      if (settings.getInt(SIGNATURE) != MAGIC) throw new IllegalStateException("No Index file, magic is not " + MAGIC);

      this.keySize = settings.getInt(KEYSIZE);
      if (keySize != 0 && this.keySize != keySize) {
        throw new IllegalStateException(
          "Invalid key size for Index file. The file is " + this.keySize + " and was expected to be " + this.keySize);
      }
