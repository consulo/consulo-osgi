...
public class OBRSAXHandler extends DefaultHandler {
  ...
  public void startElement(String uri, String localName, String qName, Attributes atts) throws SAXException {
    if (TAG_REFERRAL.equals(qName)) {
      ...
      resourceBuilder =
        new Resource.Builder().setId(atts.getValue(ATTR_RESOURCE_ID)).setPresentationName(atts.getValue(ATTR_RESOURCE_PRESENTATION_NAME))
          .setSymbolicName(atts.getValue(ATTR_RESOURCE_SYMBOLIC_NAME)).setUrl(atts.getValue(ATTR_RESOURCE_URI))
          .setVersion(atts.getValue(ATTR_RESOURCE_VERSION)).setBaseUrl(baseUrl);
    }
    else if (TAG_CAPABILITY.equals(qName)) {
      capabilityBuilder = new Capability.Builder().setName(atts.getValue(ATTR_CAPABILITY_NAME));
    }
    else if (TAG_REQUIRE.equals(qName)) {
      require = new Require(atts.getValue(ATTR_REQUIRE_NAME), atts.getValue(ATTR_REQUIRE_FILTER),
                            "true".equalsIgnoreCase(atts.getValue(ATTR_REQUIRE_OPTIONAL)));
    }
      ...
      if (capabilityBuilder != null) capabilityBuilder.addProperty(p);
    }
  }
  ...
  public void endElement(String uri, String localName, String qName) throws SAXException {
    if (TAG_CAPABILITY.equals(qName)) {
      ...
      if (!resourceListener.processResource(resourceBuilder.build())) throw new StopParseException("");
