...
public class LocalOBR extends OBR implements Refreshable {
  ...
  public void setProperties(Map<String, String> map) {
    ...
    if (localIndex.exists() && !localIndex.isFile()) {
      throw new IllegalArgumentException(
        String.format("Cannot build local repository index: '%s' already exists but is not a plain file.", localIndex.getAbsolutePath()));
    }
    cacheDir = new File(storageDir, ".obrcache");
    if (cacheDir.exists() && !cacheDir.isDirectory()) {
      throw new IllegalArgumentException(
        String.format("Cannot create repository cache: '%s' already exists but is not directory.", cacheDir.getAbsolutePath()));
    }
  }
  ...
  private void regenerateIndex() throws Exception {
    ...
    if (indexer == null) throw new IllegalStateException("Cannot index repository: no Bundle Indexer service or plugin found.");
    ...
    try {
      ...
      if (!allFiles.isEmpty()) {
        ...
        ByteArrayInputStream emptyRepo =
      ...
      for (String bsn : bsns) {
        ...
        if (versions != null) {
          for (Version version : versions) {
            ...
            if (file != null) allFiles.add(file.getCanonicalFile());
          }
        }
      }
    }
