...
public class FileRepo implements Plugin, RepositoryPlugin, Refreshable, RegistryPlugin {
  ...
  public void setProperties(Map<String, String> map) {
    ...
    if (location == null) throw new IllegalArgumentException("Location must be set on a FileRepo plugin");

    root = new File(location);
    if (!root.isDirectory()) throw new IllegalArgumentException("Repository is not a valid directory " + root);

    String readonly = (String)map.get(READONLY);
    if (readonly != null && Boolean.valueOf(readonly).booleanValue()) canWrite = false;
  ...
  public File[] get(String bsn, String versionRange) throws Exception {
    ...
    if (versionRange != null && versionRange.equals("project")) return null;
    ...
    for (int i = 0; i < instances.length; i++) {
      ...
      if (m.matches() && m.group(1).equals(bsn)) {
        ...
        if (range.includes(version) || versionString.equals(versionRange)) versions.put(version, instances[i]);
      }
    }
  ...
  public File put(Jar jar) throws Exception {
    ...
    if (manifest == null) throw new IllegalArgumentException("No manifest in JAR: " + jar);

    String bsn = manifest.getMainAttributes().getValue(Analyzer.BUNDLE_SYMBOLICNAME);
    if (bsn == null) throw new IllegalArgumentException("No Bundle SymbolicName set");

    Map<String, Map<String, String>> b = Processor.parseHeader(bsn, null);
    if (b.size() != 1) throw new IllegalArgumentException("Multiple bsn's specified " + b);

    for (String key : b.keySet()) {
      bsn = key;
      if (!Verifier.SYMBOLICNAME.matcher(bsn).matches()) throw new IllegalArgumentException("Bundle SymbolicName has wrong format: " + bsn);
    }
  ...
  protected void fireBundleAdded(Jar jar, File file) {
    ...
    for (RepositoryListenerPlugin listener : listeners) {
      try {
        ...
        if (reporter != null) reporter.warning("Repository listener threw an unexpected exception: %s", e);
      }
    }
  }
  ...
  public void setLocation(String string) {
    ...
    if (!root.isDirectory()) throw new IllegalArgumentException("Invalid repository directory");
  }
  ...
  public List<String> list(String regex) throws Exception {
    ...
    if (regex != null) pattern = Instruction.getPattern(regex);
    ...
    if (root == null) {
      ...
      if (list != null) {
        for (File f : list) {
          ...
          if (pattern == null || pattern.matches(fileName)) result.add(fileName);
        }
      }
      else if (reporter != null) reporter.error("FileRepo root directory (%s) does not exist", root);
    }
    ...
    if (dir.isDirectory()) {
      ...
      for (String v : versions) {
        ...
        if (m.matches()) {
          ...
          if (version.equals("latest")) version = Integer.MAX_VALUE + "";
  ...
  public File get(String bsn, String version, Strategy strategy, Map<String, String> properties) throws Exception {
    ...
    if (strategy == Strategy.EXACT) {
      ...
      if (vr.isRange()) return null;

      if (vr.getHigh().getMajor() == Integer.MAX_VALUE) version = "latest";
