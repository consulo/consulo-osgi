...
public class OSGiHeader {
  ...
  static public Map<String, Map<String, String>> parseHeader(String value, Reporter logger) {
    if (value == null || value.trim().length() == 0) return Create.map();
      ...
      if (name == null || name.length() == 0) {
        ...
        while (del == ';') {
          ...
          if ((del = qt.getSeparator()) != '=') {
            ...
            if (adname != null && adname.length() > 0) aliases.add(adname.trim());
          }
          else {
            ...
            if (clause.containsKey(adname)) {
              if (logger != null && logger.isPedantic()) {
                logger
                  .warning("Duplicate attribute/directive name " + adname + " in " + value + ". This attribute/directive will be ignored");
              }
            }
            if (advalue == null) {
              if (logger != null) logger.error("No value after '=' sign for attribute " + adname);
              advalue = "";
            }
  ...
  public static Map<String, String> parseProperties(String input, Reporter logger) {
    if (input == null || input.trim().length() == 0) return Create.map();
