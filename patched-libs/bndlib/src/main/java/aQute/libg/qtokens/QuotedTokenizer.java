...
public class QuotedTokenizer {
  ...
  public QuotedTokenizer(String string, String separators, boolean returnTokens) {
    if (string == null) throw new IllegalArgumentException("string argument must be not null");
    this.string = string;
  ...
  private void quotedString(StringBuffer sb, char c) {
    ...
    while (index < string.length()) {
      ...
      if (c == quote) break;
      if (c == '\\' && index < string.length() && string.charAt(index + 1) == quote) c = string.charAt(index++);
      sb.append(c);
    }
  }
  ...
  private String[] getTokens(int cnt) {
    ...
    if (token == null) return new String[cnt];
